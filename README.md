...
상단 표에만 넣는 활동은 같은 설정의 `chartAnnotations`에서 관리합니다. `id`, `category`, `title`, `startDate`, 선택적 `endDate`, `chartOnly: true`를 지정하며 날짜·분류·중복 ID를 검사합니다. 이 항목은 하단 목록·상세 경로·다음 경험 순서에 포함하지 않고 클릭 링크도 만들지 않습니다. 기존 반응형 정책에 따라 표가 숨겨지는 모바일에서는 표시하지 않습니다. 현재 Work의 `NCP AI Enablement 팀 PT`는 2026년 3월~~6월 초(표시 기준 6월 5일), Build의 `NCP 가이드 챗봇 구현`은 2026년 4~~6월에 해당합니다.
