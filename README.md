...
- 본문: 문단, 순서·비순서 목록, 굵게, 기울임, `http`·`https`·`mailto` 링크 및 로컬 자료 이미지 허용

모바일 화면을 나란히 비교하려면 연속된 이미지 문단에 `![설명](/media/experiences/<id>/screen.png "gallery")`처럼 `gallery` 제목을 붙입니다. 두 장 이상일 때만 묶이며 데스크톱에서는 2열, 480px 이하에서는 1열입니다. 일반 이미지와 기존 PPT 자료의 배치는 바뀌지 않습니다. 각 이미지의 원본 링크와 설명은 그대로 유지합니다.

자료 이미지는 `public/media/experiences/<id>/`에 넣고, 성과 아래에 `![자료 설명](/media/experiences/<id>/slide.png)`처럼 독립된 문단으로 작성합니다. 폴더·파일 이름은 소문자 영문·숫자·하이픈, 확장자는 png·jpg·jpeg·webp·gif를 사용합니다. 이미지는 원본 비율과 원본 보기 링크를 제공하며 GIF는 사용자가 재생을 눌러야 표시됩니다. Notion 이미지 가져오기는 지원하지 않으므로 자료는 로컬 Markdown에서 추가합니다. `public/`에 넣은 파일은 게시 상태와 무관하게 배포에 포함되므로 비공개 자료를 넣지 마세요.
...
상단 표에만 넣는 활동은 같은 설정의 `chartAnnotations`에서 관리합니다. `id`, `category`, `title`, `startDate`, 선택적 `endDate`, `chartOnly: true`를 지정하며 날짜·분류·중복 ID를 검사합니다. 이 항목은 하단 목록·상세 경로·다음 경험 순서에 포함하지 않고 클릭 링크도 만들지 않습니다. 기존 반응형 정책에 따라 표가 숨겨지는 모바일에서는 표시하지 않습니다. 현재 Work의 `NCP AI Enablement 팀 PT`는 2026년 3월부터 6월 초(표시 기준 6월 5일), Build의 `NCP 가이드 챗봇 구현`은 2026년 4월부터 5월 말(6월 시작 전)까지에 해당합니다.
